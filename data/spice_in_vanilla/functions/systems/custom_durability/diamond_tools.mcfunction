...
#tellraw @s {"score":{"name":"@s","objective":"DivideScore"}}
#tellraw @s {"score":{"name":"#Damage","objective":"Durability"}}
...
execute as @s if block -30000000 1 50232 minecraft:yellow_shulker_box{Items:[{Slot:0b,tag:{CustomDurability:{Value:0}}}]} store result block -30000000 1 50232 Items[0].tag.Damage int 1 run scoreboard players get #DiamondTools Durability
...
execute store result storage spice_in_vanilla:items Durability int 1 run data get block -30000000 1 50232 Items[0].tag.CustomDurability.Value
