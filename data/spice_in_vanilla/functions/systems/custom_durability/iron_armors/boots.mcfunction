execute as @s run function spice_in_vanilla:systems/modify_inventory/copy/boots

#修繕で回復したダメージ値を反映させる
scoreboard players operation @s Durability = #IronBoots Durability
...
scoreboard players operation @s DivideScore = #IronBoots Durability
...
execute as @s[scores={TakenDamage=..0}] store result block -30000000 1 50232 Items[0].tag.Damage int 1 run scoreboard players get #IronBoots Durability
...
execute as @s store result score @s Durability run data get block -30000000 1 50232 Items[0].tag.CustomDurability.Value
execute as @s[scores={Durability=0..}] run function spice_in_vanilla:systems/modify_inventory/paste/boots
